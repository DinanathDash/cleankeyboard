# Clean Keyboard, a Droplet for Droppy
...
- Droplet id: `cleankeyboard`. It is also `CleankeyboardDroplet.id` in Swift and `id` in `droplet.json`; the three must agree or the loader refuses the bundle.
- Swift product: `Cleankeyboard`, a dynamic library. The harness target is `CleankeyboardHarness`.
- SDK checkout: `/Users/dinanathdash/Documents/Projects/droppykit` (DroppyKit 1.5.0). Docs online: https://getdroppy.app/docs/droppykit
...
1. Edit `Sources/Cleankeyboard/`. The manifest is `droplet.json`.
2. `droppykit build` writes `.build/Cleankeyboard.droplet`, universal, linked against the
...
5. Put the bundle into Droppy Playground and confirm it loaded. Copy
   `.build/Cleankeyboard.droplet` to
   `~/Library/Application Support/Droppy Playground/Droplets/cleankeyboard/Cleankeyboard.droplet`,
...
- **The principal class does nothing.** `CleankeyboardPrincipal` is `@objc`, is named in the
...
- **No `main.swift`.** The harness entry is `@main` in
  `Sources/CleankeyboardHarness/CleankeyboardHarness.swift`, and a file named `main.swift` cannot
  coexist with `@main`.
...
```
Package.swift                     product Cleankeyboard (dynamic) and CleankeyboardHarness
droplet.json                      the manifest; Info.plist is generated from it
Sources/Cleankeyboard/              the droplet
Sources/CleankeyboardHarness/       the @main harness entry; never main.swift
Cleankeyboard.icon/                 Icon Composer document, required
Assets/Creator.png                square creator avatar, at least 256px, required
.build/Cleankeyboard.droplet        what droppykit build writes
